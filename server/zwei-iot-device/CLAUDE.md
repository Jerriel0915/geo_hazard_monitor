...
- `device_sensor` — 传感器 (id / deviceId / deviceCode / sensorCode UNIQUE(deviceId,sensorCode) /
...
| 2026-06-23 | **传感器编号约束改为设备内唯一**: `checkSensorCodeUnique` 签名加 `deviceId` 维度; 删除全局查询方法 `findBySensorCode` / `selectSensorByCode`; `DeviceRegistryServiceImpl.buildUniqueSensorCode` + `DeviceServiceImpl.resolveCopySensorCode` 改为设备内查重; `MonitorIngestConsumerService.resolveSensorId` 修复为复合查询 (避免跨设备同名传感器误更新); 新增 4 个边界测试用例; 升级脚本 `db/upgrade/v2.9-sensor-code-device-unique.sql` |