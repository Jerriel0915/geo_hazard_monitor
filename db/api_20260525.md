...
| 主题 | 说明 |
| --- | --- |
| `sys/v1/{device_id}/{sensor_no}/updata` | 通用 JSON 上报主题 |
| `gb/v1/{device_id}/{sensor_no}/updata` | 国标兼容上报主题 |

**发布校验规则：**

1. 仅允许发布上述两类主题。
2. `topic` 中的 `{device_id}` 必须与当前已鉴权设备一致。
3. `{sensor_no}` 必须在 `device_sensor` 中存在，且传感器状态为启用。
