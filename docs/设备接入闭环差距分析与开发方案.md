...
- 通过订阅主题区分通用格式与国标格式
- 通用格式主题保留为 `sys/v1/{device_id}/{sensor_no}/updata`
- 其中 `{device_id}` 为设备 ID，`{sensor_no}` 为传感器编号
- 取消独立的认证凭证机制
- 改为轻量、可快速扩展上线的设备账号认证
...
- 现有数据库：`db/geo_hazard_monitor_v1.5.sql`
...
- `device_id`：平台设备主键 ID，作为设备接入和主题路由中的唯一标识
- `username`：平台生成的设备接入用户名，固定 6 位
- `password`：平台生成的设备接入密码，固定 8 位

推荐规则如下：

- `device_id`：直接使用平台 `device.id`
...
- 通用格式设备发布到 `sys/v1/{device_id}/{sensor_no}/updata`
- 国标格式设备发布到 `gb/v1/{device_id}/{sensor_no}/updata`
...
- `MqttServerMessageListener` 仍只监听测试主题 `/test/${xxxx}`

结论：当前 MQTT 鉴权中心与连接治理能力已达到 P0 可生产接入水平，但消息解析、IoTDB 落库、失败补偿尚未完成，整体“设备接入闭环”仍未全部打通。
...
- 数据链路未打通
- 现有数据库中存在 `monitor_data` 表，但不符合“设备上报信息只走时序数据库”的新要求
- 告警表 `alarm_criteria`、`alarm_record` 已存在
- 后端未实现基于 IoTDB 的查询模块
- 未发现告警引擎实现
- 未发现时序库写入实现
...
当前系统数据库层面具备部分业务模型基础，但“设备上报只进时序库”的新链路尚未落地，现有 `monitor_data` 更适合作为历史遗留表逐步下线。
...
- `deviceId`：设备 ID，应与主题中的 `{device_id}` 一致
- `sensorNo`：传感器编号，应与主题中的 `{sensor_no}` 一致
...
- 设备定位优先以主题中的 `{device_id}` 为准
- 传感器定位优先以主题中的 `{sensor_no}` 为准
- payload 中的 `deviceId`、`sensorNo` 作为一致性校验字段
...
- 平台根据 `gb/v1/{device_id}/{sensor_no}/updata` 路由到国标解析器
- 国标解析器完成字段提取、标准化映射后统一写入 IoTDB

国标适配规则：

- 主题中的 `{device_id}`、`{sensor_no}` 作为设备与传感器主定位键
...
- 通用格式解析器负责处理 `sys/v1/{device_id}/{sensor_no}/updata`
- 国标解析器负责处理 `gb/v1/{device_id}/{sensor_no}/updata`
- 两类解析器最终统一输出标准时序写入模型
...
- 鉴权账号：平台为设备生成 6 位用户名
- 鉴权密码：平台为设备生成 8 位密码
- 通用主题：`sys/v1/{device_id}/{sensor_no}/updata`
- 国标主题：`gb/v1/{device_id}/{sensor_no}/updata`

推荐约束如下：

- `clientId` 可使用 `device-{device_id}`，便于连接追踪
- `username` 必须为平台设备账号
- `password` 必须为平台设备密码
- 主题中的 `{device_id}` 必须与已认证设备一致
...
| 功能缺失 | 基于订阅主题区分通用格式与国标格式的双路解析能力 | 设备实时数据上报 | 高 | 未实现 |
| 功能缺失 | 基于 IoTDB 的监测数据查询服务 | 曲线图、数据表、最新值 | 高 | 未实现 |
| 功能缺失 | 告警引擎实现 | 预警判据执行、记录生成 | 高 | 未实现 |
| 功能缺失 | 时序数据库写入与查询实现 | 长周期趋势分析 | 高 | 未实现 |
| 逻辑缺陷 | 设备新增直接使用实体入参，参数约束不完整 | 脏数据写入 | 中 | 已关闭 |
| 逻辑缺陷 | 绑定设备采用先删后插，缺少细粒度变更保护 | 安装位置信息被覆盖 | 中 | 未处理 |
| 逻辑缺陷 | 绑定时未校验设备、隐患点状态与存在性 | 关系错误 | 高 | 未处理 |
...
| 性能不足 | 缺少基于 Redis Stream 或内存队列的接入缓冲、重试、死信机制 | 大并发和网络抖动场景 | 高 | 未实现 |
| 性能不足 | 未形成面向时序数据库的分表、标签、压缩和冷热策略 | 历史曲线查询 | 中 | 未实现 |
| 安全隐患 | MQTT 鉴权中心未形成完整消息闭环，当前仅完成鉴权与权限控制 | 消息解析与落库仍待补齐 | 高 | 部分实现 |
...
| MQTT 消息采集 | 解析 `sys/v1/{device_id}/{sensor_no}/updata` 与 `gb/v1/{device_id}/{sensor_no}/updata` | 支持 QoS 0/1 | 未实现 |
| IoTDB 时序服务 | 写入 IoTDB，并提供最新值、分页、图表查询 | 完整查询链路 | 未实现 |
...
需落地以下文档已定义接口，且查询源统一为 IoTDB：

- `GET /api/v1/monitor-data/chart`
- `GET /api/v1/monitor-data/page`
- `GET /api/v1/monitor-data/latest`
...
flowchart LR
  ...
  subgraph C[采集服务]
    C1[解析topic中的device_id和sensor_no]
    C2[校验账号设备与topic中的device_id一致]
    C3[按sys或gb主题分发到解析器]
...
- `password = 8位平台生成设备密码`
- `topic = sys/v1/{device_id}/{sensor_no}/updata` 或 `gb/v1/{device_id}/{sensor_no}/updata`
...
1. 解析主题中的 `device_id`
2. 比对已鉴权设备与 `device_id` 是否一致
3. 解析 `sensor_no`
4. 根据订阅主题判断通用解析器或国标解析器
5. 根据 `device_id + sensor_no` 定位传感器
6. 再进入报文解析并写入 IoTDB

当前已落地范围：

- 已实现 `CONNECT` 鉴权、失败封禁、设备账号状态校验、协议类型校验
- 已实现发布主题准入：仅允许 `sys/v1/{device_id}/{sensor_no}/updata` 与 `gb/v1/{device_id}/{sensor_no}/updata`
- 已实现订阅主题校验：当前设备订阅校验规则为 `sys/v1/{deviceCode}/{sensorCode}/updata`
- 已实现 `topic` 中设备标识与已鉴权设备一致性校验
- 已实现测点归属和启用状态校验
- 已实现上下线状态回写与 `device_auth_log` 审计落库
- 已实现结构化 MQTT 异常体系与统一异常收敛日志输出
- 尚未实现通用/国标报文解析、Redis Stream 缓冲、IoTDB 落库和告警联动
...
- 订阅主题：`sys/v1/{device_id}/{sensor_no}/updata`
- 按 `docs/物联网报送规则设计.MD` 中定义的 JSON 结构解析
- 校验 `deviceId` 与 `{device_id}` 一致
- 校验 `sensorNo` 与 `{sensor_no}` 一致
- 将 `data.time`、`data.value` 写入 IoTDB

### 国标格式解析

- 订阅主题：`gb/v1/{device_id}/{sensor_no}/updata`
- 按国标原始报文格式解析
...
- 已执行 `mvn -pl zwei-iot test`，当前 `zwei-iot` 模块测试全部通过
