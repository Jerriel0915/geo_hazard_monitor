...
- `mqtt.server.auth.enable = false`
- `MqttServerAuthHandler.authenticate()` 直接返回 `false`
- `MqttServerMessageListener` 只监听测试主题 `/test/${xxxx}`

结论：当前 MQTT 接入能力尚未达到可生产接入水平。
...
| 安全隐患 | MQTT 鉴权默认关闭 | 非法设备可尝试接入 | 高 | 未处理 |
| 安全隐患 | 认证处理器未实现 | 实际不可控接入 | 高 | 未处理 |
...
| MQTT 鉴权中心 | Broker 启用用户名密码鉴权 | 拒绝未授权设备 | 未实现 |
...
| 设备在线状态服务 | MQTT 在线/离线、最后上报时间更新 | 与业务设备状态联动 | 未实现 |
