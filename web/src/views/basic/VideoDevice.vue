<template>
  <div class="page">
    ...
    <!-- 搜索栏 -->
    <div class="search">
      <el-input v-model="searchKeyword" placeholder="搜索编号或名称" clearable @clear="handleSearch" @keyup.enter="handleSearch" />
      <el-select v-model="searchProtocol" placeholder="选择协议" clearable @change="handleSearch">
            ...
            <template #default="{ row }">
              <div class="hazard-tags-wrapper">
                <span v-if="row.hazardPointIds && row.hazardPointIds.trim()" class="hazard-tags">
                  <el-tag v-for="hpId in row.hazardPointIds.split(',')" :key="hpId" size="small" class="hazard-tag">
          ...
          <el-col :span="12">
            <el-form-item label="图标">
              <div class="device-icon-selector" @click="handleSelectVideoIcon">
                ...
                <span v-else class="device-icon-placeholder">点击选择</span>
              </div>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="关联隐患点">
              <el-select v-model="formData.hazardPointIds" multiple filterable clearable placeholder="请选择隐患点" style="width: 100%">
                <el-option v-for="hp in hazardPointList" :key="hp.id" :label="hp.name" :value="String(hp.id)" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="安装位置">
              <el-input v-model="locationText" placeholder="经度,纬度（例如 104.063456, 30.671234）" @blur="onLocationBlur">
                <template #suffix>
                  <el-button link @click="openMapPicker" title="在地图上获取坐标" class="map-btn-inline">
    ...
    <el-dialog v-model="playDialogVisible" title="视频播放" width="900px" :close-on-click-modal="false" destroy-on-close class="video-play-dialog">
      <div class="video-container">
        <div class="video-wrapper" ref="videoWrapper">
          ...
          <div v-if="videoError" class="video-error"><span>视频加载失败</span></div>
        </div>
        <div class="video-controls">
          <el-button type="primary" size="small" @click="handlePlayPause">{{ isPlaying ? '暂停' : '播放' }}</el-button>
...
// ==================== 类型 ====================
interface HazardPointItem { id: string; code?: string; name: string; longitude?: number; latitude?: number }
...
const videoIconList = Array.from({ length: 10 }, (_, i) => ({
  ...
  name: `视频图标${i + 1}`,
  icon: `vidio${i + 1}`,
  path: `/jc-icon/green/vidio${i + 1}_green.png`
}))
...
const mapInitialPoint = computed<LatLng | null>(() =>
  formData.longitude != null && formData.latitude != null ? { lng: formData.longitude, lat: formData.latitude } : null
)
...
// ==================== 工具函数 ====================
const getProtocolType = (code: string) => ({ RTMP: 'success', RTSP: 'warning', ONVIF: 'info' }[code] || 'default')
...
const loadHazardPointList = async () => {
  try {
    ...
    hazardPointList.value = [
      ...
      { id: '3', name: '边坡监测点' }, { id: '4', name: '泥石流隐患点' },
      { id: '15', name: '隐患点15' }, { id: '16', name: '隐患点16' }
    ]
  }
}
...
const loadTableData = async () => {
  loading.value = true
  try {
    const params: VideoDevicePageParams = { pageNum: currentPage.value, pageSize: pageSize.value }
    if (searchKeyword.value) { params.code = searchKeyword.value; params.name = searchKeyword.value }
    if (searchProtocol.value) params.protocolCode = searchProtocol.value
...
// ==================== 事件处理 ====================
const handleSearch = () => { currentPage.value = 1; loadTableData() }
const handleReset = () => { searchKeyword.value = ''; searchProtocol.value = ''; currentPage.value = 1; loadTableData() }
...
const handleAdd = () => {
  ...
  isEdit.value = false
  Object.assign(formData, { id: undefined, code: '', name: '', icon: '', iconPath: '', protocolCode: '', streamUrl: '', longitude: null, latitude: null, hazardPointIds: [], originalHazardPointIds: [] })
  locationText.value = ''
...
const handleEdit = async (row: VideoDeviceItem) => {
  ...
  try {
    ...
    if (res.code === 200 && res.data) {
      ...
      const initialHpIds = detail.hazardPointIds ? detail.hazardPointIds.split(',').filter(Boolean) : []
      Object.assign(formData, {
        id: detail.id, code: detail.code, name: detail.name,
        icon: detail.icon || '', iconPath: detail.iconPath || '',
        protocolCode: detail.protocolCode, streamUrl: detail.streamUrl,
        longitude: detail.longitude ?? null, latitude: detail.latitude ?? null,
        hazardPointIds: initialHpIds, originalHazardPointIds: [...initialHpIds]
      })
      syncFormToText()
    }
    dialogVisible.value = true
...
const handleDelete = (row: VideoDeviceItem) => {
  ElMessageBox.confirm(`确定要删除视频设备"${row.name}"吗?`, '删除确认', { type: 'warning' })
    .then(async () => {
      const res = await deleteVideoDevice(row.id)
      if (res.code === 200) { ElMessage.success('删除成功'); loadTableData() }
...
const syncFormToText = () => {
  locationText.value = (formData.longitude != null && formData.latitude != null)
    ? `${Number(formData.longitude).toFixed(6)}, ${Number(formData.latitude).toFixed(6)}`
    : ''
}

const onLocationBlur = () => {
  const raw = locationText.value.trim()
  if (!raw) { formData.longitude = null; formData.latitude = null; return }
  const parts = raw.split(/[,，\s]+/)
  if (parts.length >= 2) {
    const lng = Number(parts[0]); const lat = Number(parts[1])
    if (!isNaN(lng) && !isNaN(lat) && lng >= -180 && lng <= 180 && lat >= -90 && lat <= 90) {
      formData.longitude = lng; formData.latitude = lat
...
const openMapPicker = () => { onLocationBlur(); mapDialogVisible.value = true }
const onMapConfirm = (point: LatLng) => { formData.longitude = point.lng; formData.latitude = point.lat; syncFormToText() }

const syncHazardPointAssociations = async () => {
  const oldIds = formData.originalHazardPointIds, newIds = formData.hazardPointIds, deviceId = Number(formData.id)
  const toUnbind = oldIds.filter(id => !newIds.includes(id))
  for (const hpId of toUnbind) await unbindVideoDevicesFromHazardPoint(hpId, [deviceId]).catch(() => {})
  const toBind = newIds.filter(id => !oldIds.includes(id))
  for (const hpId of toBind) await bindVideoDevicesToHazardPoint(hpId, { videoDeviceIds: [deviceId] }).catch(() => {})
}

const bindHazardPointsToVideoDevice = async (videoDeviceId: string) => {
  const deviceId = Number(videoDeviceId)
  for (const hpId of formData.hazardPointIds) {
    await bindVideoDevicesToHazardPoint(hpId, { videoDeviceIds: [deviceId] }).catch(() => {})
  }
}
    ...
    try {
      if (formData.id) {
        const res = await updateVideoDevice(formData.id, {
          name: formData.name, icon: formData.icon, iconPath: formData.iconPath,
          protocolCode: formData.protocolCode, streamUrl: formData.streamUrl,
          longitude: formData.longitude, latitude: formData.latitude
        })
        if (res.code === 200) {
          await syncHazardPointAssociations()
          ElMessage.success('修改成功'); dialogVisible.value = false; loadTableData()
        } else {
          ElMessage.error(res.msg || '修改失败')
        }
      } else {
        const res = await createVideoDevice({
          code: formData.code, name: formData.name, icon: formData.icon, iconPath: formData.iconPath,
          protocolCode: formData.protocolCode, streamUrl: formData.streamUrl,
          longitude: formData.longitude, latitude: formData.latitude, status: 1
        })
        if (res.code === 200) {
          await bindHazardPointsToVideoDevice(String(res.data))
          ElMessage.success('新增成功'); dialogVisible.value = false; loadTableData()
        } else {
...
const handleSelectVideoIcon = () => { videoIconDialogVisible.value = true }
const handleVideoIconSelect = (item: { code: string; name: string; icon: string; path: string }) => {
  formData.icon = item.icon; formData.iconPath = item.path; videoIconDialogVisible.value = false
}

// ==================== 视频播放 ====================
const handlePlay = (row: VideoDeviceItem) => { playUrl.value = row.streamUrl; videoLoaded.value = false; videoError.value = false; playDialogVisible.value = true }
...
const handleScreenshot = () => {
  if (!videoRef.value || videoRef.value.videoWidth === 0 || videoRef.value.videoHeight === 0) {
    ElMessage.warning('视频未开始播放或尺寸无效')
    return
  }
  try {
    ...
    if (!ctx) { ElMessage.error('无法创建截图'); return }
    ...
    ElMessage.error('截图失败，可能由于跨域限制')
  }
}
const handleRefresh = () => { if (videoRef.value) { videoLoaded.value = false; videoError.value = false; videoRef.value.load() } }
const onVideoLoaded = () => { videoLoaded.value = true; videoError.value = false }
const onVideoError = () => { videoLoaded.value = false; videoError.value = true }

// ==================== 生命周期 ====================
onMounted(() => { loadTableData(); loadHazardPointList() })
</script>

<style scoped>
.table-icon { width: 28px; height: 28px; object-fit: contain; }
.empty-text { color: #909399; }
.hazard-tags { display: flex; flex-wrap: wrap; gap: 4px; }
.hazard-tag { margin: 2px; }
.hazard-tags-wrapper { display: flex; justify-content: center; align-items: center; flex-wrap: wrap; gap: 4px; }
.stream-url { font-size: 12px; color: #606266; word-break: break-all; }
...
.device-icon-selector:hover { border-color: #1890ff; background: #e6f7ff; }
.device-icon-img { width: 28px; height: 28px; object-fit: contain; }
.device-icon-placeholder { color: #909399; font-size: 12px; }

.icon-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px; padding: 10px; }
.icon-item {
  display: flex; flex-direction: column; align-items: center; justify-content: center;
  padding: 10px; border: 1px solid #e8e8e8; border-radius: 4px;
  cursor: pointer; transition: all 0.3s;
}
.icon-item:hover { border-color: #1890ff; background: #e6f7ff; }
.icon-select-img { width: 32px; height: 32px; object-fit: contain; }
.icon-name { font-size: 12px; color: #606266; margin-top: 6px; text-align: center; }

.video-play-dialog :deep(.el-dialog__body) { padding: 10px; }
.video-container { display: flex; flex-direction: column; align-items: center; }
.video-wrapper { width: 100%; height: 480px; background: #000; position: relative; display: flex; align-items: center; justify-content: center; }
.video-player { width: 100%; height: 100%; object-fit: contain; }
.video-loading, .video-error { position: absolute; display: flex; flex-direction: column; align-items: center; color: #fff; }
.video-loading { gap: 10px; }
.custom-spinner {
  width: 24px; height: 24px;
  border: 3px solid rgba(255,255,255,0.3); border-top-color: #409eff;
  border-radius: 50%; animation: spin 0.8s linear infinite;
}
@keyframes spin { to { transform: rotate(360deg); } }
.video-controls { display: flex; gap: 10px; margin-top: 15px; }
:deep(.el-form-item) { margin-bottom: 18px; }

/* 地图按钮在框内无背景 */
.map-btn-inline {
  padding: 0 !important; border: none !important; background: transparent !important;
  color: #606266 !important; height: 100% !important;
  display: flex !important; align-items: center !important;
}
.map-btn-inline:hover { color: #1890ff !important; background: transparent !important; }
</style>